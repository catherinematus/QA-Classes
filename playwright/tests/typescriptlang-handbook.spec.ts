...
test.describe('Typescript Official Site Tests - Handbook Page', async () => {
  ...
  for (const item in HANDBOOK_SIDEBAR_ITEMS) {
    ...
    test(`${buttonName} sidebar item state should be correct by default`, async () => {
      const navigationButtonElement = handbookPage.getSideNavigationItemByInnerText(buttonName);
  ...
  test("Should display the feedback section text correctly depending on the feedback given", async () => {
    await expect(handbookPage.getFeedbackElement()).toHaveText("Is this page helpful?");
    await handbookPage.leaveFeedback(FEEDBACK_TYPES.LIKE);
    await expect(handbookPage.getFeedbackElement()).toHaveText("Thanks for the feedback");
  });
